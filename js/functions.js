...
let questionWiev = function(){
    question.style.transform = 'translateX(0px)';
}

robotView();

let cls1 = document.querySelector('#robot .cls');
...
/////////////////////////////////////
// анімайійний робот справа екрану //
/////////////////////////////////////

let robot = document.getElementById('robot'),
    close = robot.getElementsByClassName('close')[0],
    question = document.getElementById('question'),
    robotLink = document.getElementsByClassName('wiggle')[0],
    customDate = new Date();


    question.onclick = () => {
        question.style.transform = 'translateX(250px)';
        robot.style.transform = 'translateX(0px)';    
    }

    close.onclick = () => {
        robot.style.transform = 'translateX(150px)';
        question.style.transform = 'translateX(150px)';
        clearTimeout(timer);
    }

    // якщо клікнути на робота (перехід на сторінку ФАК)
    let setMemory = () => {
        robotLink.onclick = () => {
            localStorage.setItem('customDate', customDate.getTime());
            robot.style.transform = 'translateX(150px)';
            question.style.transform = 'translateX(150px)';
            clearTimeout(timer);                    
        }
    ...
    // якщо є мітка в пам’яті (не 1 запуск)
    if(localStorage.getItem('customDate') !== null){

        // якщо час мітки вийшов
        if((+localStorage.getItem('customDate') + 86400000) < +(customDate.getTime())){ // 86400000 -- доба
        ...
        // якщо час мітки ще не вийшов
        } else {
            question.style.transform = 'translateX(150px)';
        }

    // якщо в пам’яті мітки немає (1 запуск)
    } else {
